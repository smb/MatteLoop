# MatteLoop Complete Implementation Plan
...
**Architecture:** `matteloop.core` contains immutable domain state and pure media algorithms, `matteloop.jobs` owns I/O, persistence, downloads, subprocess isolation, and rendering, and `matteloop.ui` renders state and emits typed events. Qt widgets and `QPixmap` remain on the GUI thread; ONNX session creation/inference runs in one spawn-based child process; every render consumes an immutable request and promotes durable cut frames before encoding.
...
**Spec:** `docs/designs/matteloop-desktop-app.md`
...
| Path | Responsibility |
|---|---|
| `src/matteloop/core/specs.py` | Frozen validated sampling/crop/segmentation/framing/output requests. |
| `src/matteloop/core/state.py` | Immutable app state, typed events, reducer, derived capabilities. |
| `src/matteloop/core/errors.py` | Serializable stable error codes and user-facing recovery metadata. |
| `src/matteloop/core/timebase.py` | Rational sampling and WebP delay projection. |
| `src/matteloop/core/fingerprints.py` | Targeted preview/cut/render/source fingerprints. |
| `src/matteloop/core/geometry.py` | Pure crop transforms, alpha union, framing, and UI interaction geometry. |
| `src/matteloop/core/webp.py` | Lossless encode, validation, and bounded size fitting. |
| `src/matteloop/jobs/source.py` | PyAV source probe and exact-frame decode with private containers. |
| `src/matteloop/jobs/thumbnails.py` | Cancelable target-scaled `QImage` thumbnail requests and cache metadata. |
| `src/matteloop/jobs/context.py` | Job identity, cancellation, progress, and exclusive scheduling. |
| `src/matteloop/jobs/segmentation_host.py` | Spawn-safe Pipe/shared-memory protocol and reusable rembg session. |
| `src/matteloop/jobs/models/` | Pinned 15-ID catalog and verified local downloads/cache/sessions. |
| `src/matteloop/jobs/workspace.py` | Durable cuts, manifests, validation, promotion, external-edit detection, snapshots. |
| `src/matteloop/jobs/render.py` | Preview, render, Rebuild, staging, validation, atomic output. |
| `src/matteloop/ui/` | Main window, presenters, canvases, timeline/crop, accessibility, inspector, dialogs. |
...
- Create: `.gitignore`
- Create: `README.md`
- Create: `src/matteloop/__init__.py`
- Create: `src/matteloop/__main__.py`
- Create: `src/matteloop/app.py`
...
**Interfaces:**
- Produces: `matteloop.app.main(argv: Sequence[str] | None = None) -> int`.
...
```bash
git add .python-version pyproject.toml uv.lock .gitignore README.md src/matteloop tests .github/workflows/ci.yml
git commit -m "feat: scaffold MatteLoop project"
```

### Task 2: Validated immutable domain contracts and structured errors

**Files:**
- Create: `src/matteloop/core/__init__.py`
- Create: `src/matteloop/core/errors.py`
- Create: `src/matteloop/core/specs.py`
...
```bash
git add src/matteloop/core tests/core/test_specs.py tests/core/test_errors.py
...
**Files:**
- Create: `src/matteloop/core/state.py`
...
```bash
git add src/matteloop/core/state.py tests/core/test_state.py
...
**Files:**
- Create: `src/matteloop/core/timebase.py`
- Create: `src/matteloop/core/fingerprints.py`
...
```bash
git add src/matteloop/core/timebase.py src/matteloop/core/fingerprints.py tests/core
...
**Files:**
- Create: `src/matteloop/core/geometry.py`
- Create: `tests/core/test_geometry.py`
- Create: `tests/core/test_framing.py`

**Interfaces:**
- Produces: pure frozen `PointF`, `SizeF`, `RectF`, `MediaTransform`, `InteractionGeometry`, `build_crop_geometry`, `build_timeline_geometry`; `matteloop.core` imports no PySide module.
...
```bash
git add src/matteloop/core/geometry.py tests/core/test_geometry.py tests/core/test_framing.py
...
**Files:**
- Create: `src/matteloop/core/webp.py`
...
```bash
git add src/matteloop/core/webp.py tests/core/test_webp.py
...
**Files:**
- Create: `src/matteloop/jobs/__init__.py`
- Create: `src/matteloop/jobs/source.py`
- Create: `src/matteloop/jobs/thumbnails.py`
- Create: `src/matteloop/jobs/cache.py`
...
```bash
git add src/matteloop/jobs tests/jobs/test_source.py tests/jobs/test_thumbnails.py
...
**Files:**
- Create: `src/matteloop/jobs/context.py`
- Create: `src/matteloop/jobs/protocol.py`
- Create: `src/matteloop/jobs/segmentation_host.py`
...
```bash
git add src/matteloop/jobs tests/jobs
...
- Create: `src/matteloop/jobs/models/__init__.py`
- Create: `src/matteloop/jobs/models/catalog.py`
- Create: `src/matteloop/jobs/models/download.py`
- Create: `src/matteloop/jobs/models/session.py`
...
```bash
git add resources/model-manifest.json src/matteloop/jobs/models tests/jobs/models
...
- Modify: `src/matteloop/app.py`

**Interfaces:**
- Produces: `matteloop --smoke-test` checks Qt plugin loading, PyAV decode, two-frame alpha WebP encode/reopen, spawn child, shared-memory cleanup, and optional cached fake session without downloading weights.
...
Run: `uv run python -m matteloop --smoke-test`
...
```bash
git add packaging tests/release .github/workflows/release.yml src/matteloop/app.py
...
**Files:**
- Create: `src/matteloop/jobs/workspace.py`
...
- Directory contract: `<output>/.matteloop-work/scratch/<job-id>/` disposable; `cuts/<cache-key>/` durable and never auto-deleted.
...
```bash
git add src/matteloop/jobs/workspace.py tests/jobs/test_workspace.py
...
**Files:**
- Create: `src/matteloop/jobs/render.py`
...
```bash
git add src/matteloop/jobs/render.py tests/jobs
...
**Files:**
- Create: `DESIGN.md`
- Create: `src/matteloop/ui/__init__.py`
- Create: `src/matteloop/ui/theme.py`
- Create: `src/matteloop/ui/presenter.py`
- Create: `src/matteloop/ui/main_window.py`
- Create: `src/matteloop/ui/source_strip.py`
- Create: `src/matteloop/ui/preview_canvas.py`
- Create: `src/matteloop/ui/inspector.py`
- Create: `src/matteloop/ui/action_shelf.py`
...
```bash
git add DESIGN.md src/matteloop/ui src/matteloop/app.py tests/ui
...
**Files:**
- Create: `src/matteloop/ui/geometry.py`
- Create: `src/matteloop/ui/timeline.py`
- Create: `src/matteloop/ui/crop_canvas.py`
- Create: `src/matteloop/ui/accessibility.py`
...
- Modify: `src/matteloop/ui/main_window.py`
...
```bash
git add src/matteloop/ui tests/ui
...
**Files:**
- Create: `src/matteloop/ui/job_dialog.py`
- Create: `src/matteloop/ui/model_manager.py`
- Create: `src/matteloop/ui/workspace_panel.py`
- Create: `src/matteloop/ui/error_dialog.py`
...
**Interfaces:**
- Produces: complete local command `uv run pytest`, launch command `uv run python -m matteloop`, frozen `--smoke-test`, and documented manual release record.
...
Run: `QT_QPA_PLATFORM=offscreen uv run python -m matteloop --smoke-test`
...
git commit -m "test: qualify complete MatteLoop workflow"
```
...
- Type consistency: downstream tasks consume the exact interfaces declared by earlier tasks; `matteloop.core` remains PySide-free; Qt image/widget ownership begins only in jobs/UI adapters; render parity reads the public promoted-cut workspace API.
