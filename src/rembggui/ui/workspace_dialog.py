...
from rembggui.core.errors import AppError
...
from rembggui.ui.workspace_presentation import present_workspace
...
class WorkspacePickerDialog(QDialog):
    ...
    def __init__(
        self, catalog: ModelCatalog, parent: QWidget | None = None
    ) -> None:
        super().__init__(parent)
        self.setObjectName("workspace_picker")
        self.setWindowTitle("Promoted cut sets")
        self.setAccessibleName("Promoted cut sets")
        self.resize(980, 360)
        ...
        self.cut_set_list.setAccessibleName("Promoted cut sets")
        ...
        self.use_button.setAccessibleName("Use selected cut set")
        self.open_button = QPushButton("Open folder")
        self.open_button.setAccessibleName("Open selected cut folder")
        self.delete_button = QPushButton("Delete set")
        self.delete_button.setAccessibleName("Delete selected cut set")
        self.close_button = QPushButton("Close")
        layout = QVBoxLayout(self)
        layout.addWidget(
            QLabel("Choose a validated cut set for this output directory.")
        )
        ...
        self.cut_set_list.itemDoubleClicked.connect(
    ...
    def load(self, output_directory: Path) -> None:
        ...
        try:
            ...
            self._summaries = ()
            self._message.setText(f"Could not read cut sets: {error}")
        ...
        self._message.setText(
            f"{len(self._summaries)} promoted set(s) in {output_directory}"
            if self._summaries
            else "No promoted cut sets found."
        )
        self._update_actions()
